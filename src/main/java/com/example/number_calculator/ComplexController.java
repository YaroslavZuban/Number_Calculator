...
/**
 * реализации функциональности калькулятора с поддержкой комплексных чисел. Он включает в
 * себя набор элементов управления, таких как кнопки, текстовые поля и изображения, а
 * также методы для обработки пользовательского ввода и выполнения математических операций с комплексными числами.
 *
 * Код включает в себя класс "ComplexController", который реализует интерфейс "IController" и
 * содержит аннотации "@FXML", которые указывают на элементы управления в файле FXML разметки.
 * Эти элементы управления включают кнопки для ввода чисел и операторов, а также кнопки для
 * выполнения математических операций, таких как сложение, вычитание, умножение и деление.
 *
 * Код также включает в себя методы для обработки пользовательского ввода, такие как "onNumberClicked"
 * для ввода чисел и "onOperatorClicking" для ввода операторов. Есть также методы для выполнения
 * математических операций, таких как "actionOperator" для выполнения операций над комплексными числами
 * и "result" для вычисления результата выражения. Кроме того, есть методы для работы с памятью, такие
 * как "onMemory" для сохранения числа в память и "onMdlClicked" для получения модуля комплексного числа.
 *
 * Код также включает в себя инициализацию элементов управления и обработку событий мыши для перемещения
 * окна приложения. Кнопки "buttonClose" и "buttonCollapse" используются для закрытия и сворачивания окна
 * соответственно. Кнопка "buttonHistory" открывает окно истории вычислений, а кнопка "buttonInfo"
